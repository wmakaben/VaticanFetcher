...
package net.sourceforge.vaticanfetcher.build;
...
import net.sourceforge.vaticanfetcher.build.U.LineSep;
import net.sourceforge.vaticanfetcher.man.Manual;
...
public final class BuildMain {
	...
	public static final String appName = "VaticanFetcher";
	...
	public static void main(String[] args) throws Exception {
		...
		U.copyBinaryFile("dist/" + eplFilename, "build/tmp/licenses/vaticanfetcher/" + eplFilename);
	...
	private static File recreateJarFile(String jarPrefix, boolean isPortable, LineSep lineSep) throws Exception {
		...
		File mainJarFile = new File(String.format("build/tmp/%s%s_%s_%s.jar", jarPrefix, packageId, version, buildDate));
		mainJarFile.delete();
	...
	private static void createPortableBuild(File tmpMainJar) throws Exception {
		...
		U.copyTextFile("dist/launchers/launcher-linux.sh", linuxLauncher, LineSep.UNIX,	"${main_class}", Main.class.getName());

		// Create vaticanfetcher.app launcher for Mac OS X
		...
		U.copyBinaryFile("dist/launchers/VaticanFetcher-512.exe", exeLauncher);
		...
		U.copyTextFile("dist/launchers/launcher.bat", batLauncher, LineSep.WINDOWS,	"${main_class}", Main.class.getName());
		...
		/* Create an empty file 'indexes/.indexes.txt' to let the daemons know we're the portable version. */
	...
	private static void createMacOsXBuild(File tmpMainJar) throws Exception {
		...
		U.copyTextFile("dist/launchers/launcher-macosx-app.sh", launcher, LineSep.UNIX, "${app_name}", appName,	"${main_class}", Main.class.getName());
	...
	private static void updateManualVersionNumber(File helpDir) throws Exception {
		for (File dir : Util.listFiles(helpDir)) {
			File manFile = new File(dir, "VaticanFetcher_Manual.html");
