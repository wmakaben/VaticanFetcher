...
public final class UtilGlobal {
	...
	public static <S> Iterable<S> convert(@Nullable final Iterable<? extends S> iterable) {
		...
		return new Iterable<S> () {
			public Iterator<S> iterator() {
				/* The inner iterator must be instantiated inside the iterator() method, because we need a new inner iterator every time iterator() is called. */
	...
	@NotNull
	public static String replace(@NotNull String srcPath, @NotNull String input, @NotNull String... replacements) {
	...
	@NotNull
	public static String replaceSilently(@NotNull String input,	@NotNull String... replacements) {
		Util.checkThat(replacements.length % 2 == 0);
		for (int i = 0; i < replacements.length; i += 2) {
			String s1 = replacements[i];
			String s2 = replacements[i + 1];
