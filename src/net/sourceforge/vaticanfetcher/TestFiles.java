...
public enum TestFiles {
	...
	archive_entry_7z_zip_rar ("zip-rar.7z/"	+ "test.7z/archive.zip/" + "test.zip/archive.rar/" + "test.rar/test2.txt"),
	archive_entry_zip_zip ("zip.zip/test.zip/test.txt"),
	multi_page_pdf ("multi-page.pdf"),
	encrypted_pdf ("encrypted-(password='vaticanfetcher').pdf"),
	html ("test.html"),
	...
	umlauts ("umlauts.7z/umlauts-%s/archive-%s.rar/archive-%s.7z/archive-%s.zip/file-%s.txt".replaceAll("%s", "\u00E4\u00F6\u00FC\u00DF")),
	fake_zip_file_inside_solid ("fake-zip-file-inside-solid.7z"),
	textmaker_rtf ("textmaker-rtf"),
			
	encrypted_7z ("7z-encrypted-(password='vaticanfetcher').7z"),
	encrypted_7z_filenames ("7z-encrypted-including-filenames-(password='vaticanfetcher').7z"),
	...
	private TestFiles(@NotNull String path) { this.path = Util.checkNotNull(path); }
	
	@NotNull
	public File get() {	return new File(getPath()); }
	
	@NotNull
	public String getPath() {return Util.joinPath("dev/test-files", path);}
