...
public final class IndexingQueue {
	...
	public IndexingQueue(@NotNull final IndexRegistry indexRegistry, int reporterCapacity) {
		...
		/*
		 * In case of rebuild tasks, if a task is removed before it has entered the indexing state, put the associated index back into the registry.
		 * If the task has already entered the indexing state, putting the index back into the registry is the responsibility of the worker thread.
		 */
	...
	private boolean threadLoop() {
		...
		if (task.is(IndexAction.REBUILD)) {
			/*
			 * If the task is a rebuild, the searcher will be holding on to the underlying index at this point, since 
			 * it doesn't care whether the index was removed from the registry or not. Therefore, before clearing the 
			 * index, we must signal the searcher to let go of it by refreshing the searcher's internal Lucene searcher.
			 */
		...
		try {
			if (task.is(IndexAction.UPDATE)) {
				/*
				 * In case of index updates, save to disk even if the indexing fails. An alternative is to automatically discard the index,
				 * which would probably confuse and/or annoy the user, because then indexes could magically disappear at any time.
				 */
				assert !task.is(CancelAction.DISCARD);
				if (task.getDeletion() == null || result != IndexingResult.SUCCESS_UNCHANGED) {
					/*
					 * Bug #3519920: The index should not be saved here if it's not in the registry anymore, 
					 * otherwise the program will crash when the following happens: 
					 * (1) The index is updated. 
					 * (2) While the update is running, the user requests a rebuild on the index. This causes 
					 * the index to be removed from the registry, and the update to be cancelled. 
					 * (3) The canceling gets us to this point prematurely. We erroneously save the index, which puts it back into the registry. 
					 * (4) Some time later, the rebuild task is executed. It contains the assertion that the rebuilt index isn't in the 
					 * registry anymore. Since we just put the index back in the registry, the assertion fails and crashes the program.
					 */
		...
		if (doDelete) {
			/*
			 * Note: This is a typical check-then-act situation that would normally require holding the lock. However, in 
			 * this case the lock is not needed: The task must either have been removed from the task queue when the lock was 
			 * held, or it was not an update task. In both cases, changing the deletion field at this point should not be possible.
			 */
	...
	@ThreadSafe
	private void assertValidRegistryState(@NotNull IndexRegistry indexRegistry, @NotNull Task task) {
		LuceneIndex luceneIndex = task.getLuceneIndex();
	...
	public Rejection addTask(@NotNull LuceneIndex index, @NotNull IndexAction action) {
		...
		try {
			...
			if (task.is(IndexAction.UPDATE)) {
				/* Reject update requests for indexes that are not (or no longer) in the registry. Such requests may be caused by obsolete folder watching events. */
				if (!indexesInRegistry.contains(index))
					return Rejection.INVALID_UPDATE;

				/*
				 * Here, we reject a request to enqueue an update task if there is already another task 
				 * in the queue that has the same target file or directory and is in ready state.
				 * 
				 * This is not a bullet-proof way to avoid unnecessary updates while ensuring that any necessary updates are run: Going
				 * through the queue, we could find a ready task with a matching target, and reject the enqueue request based on that, but the
				 * user could later cancel the ready task, thus skipping an update that should have been run. However, the approach here
				 * should work well enough, assuming that it is very unlikely that the user will cancel ready tasks.
				 */
				for (Task queueTask : tasks)
					if (queueTask.is(TaskState.READY) && sameTarget(queueTask, task))
				...
				/*
				 * Reject a request to create or rebuild an Outlook index if it has the same PST file as an Outlook index in the queue. 
				 * Exception: Rebuild tasks may replace existing update tasks on the same PST file, causing the update tasks to be cancelled.
				 */
				...
				/* Reject a request to create or rebuild a file index if it overlaps with a file index in the registry. */
				...
				/*
				 * Reject a request to create or rebuild a file index if it overlaps with a file index in the queue. Exception:
				 * Rebuild tasks may replace existing update tasks on the same index, causing the update tasks to be cancelled.
				 */
				Iterator<Task> it = tasks.iterator();
				while (it.hasNext()) {
					Task queueTask = it.next();
	...
	private boolean removeAll(@NotNull CancelHandler handler, @NotNull LazyList<Task> removedTasks) {
		/* Cancel active task if there is one. Note that if the cancel handler returns null, no tasks are removed. */
		for (Task task : tasks) {
	...
	// Event listeners are notified without lock and possibly from a non-GUI thread. Handler is called without lock 
	// and in the same thread as the caller of this method. The list of tasks given to the handler is an immutable copy
	@ThreadSafe
	...
	// should only be called with indexes that are currently in the registry approves immediately if no matching task is found
	// if matching inactive task is found, approval is given after removing the task from the queue
	// if matching active task is found, the task is canceled, and the approval is given after the indexing operation has finished
	@ThreadSafe
		...
		try {
			for (PendingDeletion deletion : deletions) {
				...
				while (it.hasNext()) {
					...
					/*
					 * Remove the task from the queue even if it was in indexing state - we don't want any ghost tasks to linger in the
					 * queue, since they might (theoretically) cause subsequent task addition request to fail due to directory overlaps.
					 */
					it.remove();
	...
	// returns 'proceed'; cancel handler is called if a creation or rebuild task is currently running
	// Warning: Cancel handler is called under lock, so caller must take possible lock-ordering deadlocks into account.
	/* This method must not be called again after a previous call has set the shutdown flag. */
	@ThreadSafe
	public boolean shutdown(@NotNull final CancelHandler handler) {
		...
		try {
			...
			/*
			 * Wake up and terminate worker thread if it was waiting. Do *not* call thread.interrupt here, 
			 * otherwise we'll get an exception when trying to close the current Lucene index, if there is one.
			 */
			readyTaskAvailable.signal();
		}
		finally {
