...
public enum Fields {
	...
	/*
	 * Note: All information that will be displayed on the result table must be stored in the 
	 * Lucene index (via Store.YES), because the user might have indexed a document repository 
	 * on a removable media, which may or may not be available during searches. On the other hand, 
	 * the information that is displayed on the preview panel should be loaded from the document
	 * repository rather than the Lucene index in order to display up-to-date content.
	 */
	...
	private Fields(@NotNull Store store, @NotNull Index index) {
	...
	// Will create a tokenized and indexed field that is not stored if the given fieldValue is not a String
	...
	@NotNull
	public static Field createContent(@NotNull CharSequence fieldValue,	boolean withOffsets) {
		...
		if (fieldValue instanceof String) {
			return new Field(CONTENT.key, (String) fieldValue, CONTENT.store, CONTENT.index,	termVector);
		}
		return new Field(CONTENT.key, new CharSequenceReader().setInput(fieldValue), termVector);
	}
