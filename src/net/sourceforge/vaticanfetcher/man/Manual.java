...
package net.sourceforge.vaticanfetcher.man;
...
import net.sourceforge.vaticanfetcher.build.BuildMain;
...
public final class Manual {
	...
	private enum SubPageProperties {
		...
		Memory_Limit ("memory_limit", "Other_Notes"),
	...
	public static void main(String[] args) throws Exception {
		...
		for (File file : Util.listFiles(new File(manDir + "/all"))) {
			...
			String dstPath = dstDir.getPath() + "/VaticanFetcher_Manual_Files/" + name;
	...
	@RecursiveMethod
	private static void convert(@NotNull PegDownProcessor processor, @NotNull PageProperties props,	@NotNull File srcDir,
								@NotNull File dstDir, boolean isTopLevel) throws IOException {
	...
	private static String convertMainPage(@NotNull PageProperties props, @NotNull File file, @NotNull String htmlBody) throws IOException {
		...
		Pair[] pairs = new Pair[] {
			props.vaticanfetcherManual, props.author, props.mainFooter };
		...
		return UtilGlobal.replace(path, template, "${app_name}", BuildMain.appName, "${contents}", htmlBody);
	}
	...
	private static String convertSubPage(@NotNull PageProperties props, @NotNull File file, @NotNull String htmlBody)throws IOException {
		...
		Pair[] pairs = new Pair[] {props.vaticanfetcherManual, props.author, props.backToMainPage };
		for (Pair pair : pairs) {
			String key = "${" + pair.key + "}";
			template = UtilGlobal.replace(path, template, key, pair.value);
		}
		...
		String link = "../VaticanFetcher_Manual.html#" + subProps.backlink;
	...
	private static final class PageProperties {
		...
		public final Pair vaticanfetcherManual;
		...
		public PageProperties(@NotNull File propsFile) throws IOException {
			...
			vaticanfetcherManual = getPair("vaticanfetcher_manual");
			...
			mainFooter.value = vaticanfetcherManual.value + " &ndash; " + mainFooter.value;
		}
		
		@NotNull
		private Pair getPair(@NotNull String key) throws IOException {	return new Pair(key, getValue(key));  }
